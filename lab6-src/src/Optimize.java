  ...
  public static Expression optimize(Expression expr) {
    return expr.accept(new Expression.Visitor<Expression>() {
      ...
      public Expression visit(Expression.Add expr) {
        ...
        {
            int newV = 0;
            if (expr.id.image.toString().equals("+"))
            {
                newV = ((Expression.Int)expr.left).value + ((Expression.Int)expr.right).value;
      ...
      public Expression visit(Expression.Negative expr) {
        ...
        //do the same with double
        if (expr.expr instanceof Expression.Double) {
          double newV = ((Expression.Double)expr.expr).value * -1;
          return new Expression.Double(ID(newV + ""));
        }
        return expr;
      }
